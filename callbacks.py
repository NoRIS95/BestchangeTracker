...
def get_best_rate(exchanger_name: str, get_currency_name: str, give_currency_name: str):
    try:
        ...
        return
    get_cur_ids = {cur['id'] for cur in api.currencies().search_by_name(get_currency_name).values() \
                  if get_currency_name in cur['name']}
    give_cur_ids = {cur['id'] for cur in api.currencies().search_by_name(give_currency_name).values() \
                   if give_currency_name in cur['name']}
    rates = [r['rate'] for r in api.rates().get() if r['exchange_id'] == exchanger_id and r['give_id'] \
...
def get_top(n=10, money_list=['USDT', 'RUB'], cript_list=['BTC', 'ETH', 'TON', 'XMR', 'TRX']):
    ...
    currencies_dict = api.currencies().get()
